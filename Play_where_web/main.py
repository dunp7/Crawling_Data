...
hi = load_lottieurl("https://assets1.lottiefiles.com/packages/lf20_puciaact.json")
...
def display_map(id):
    ...
    map = folium.Map(location=[21.027763, 105.834160],zoom_start= 11)
...
def choose_filter_for_random_game_choice(i):
    ...
    if df_selection.empty:
        st.write("Sorry, there's no games matched your selection")
    else:
        df_selection = df_selection.sample()
        with col2:
            st.header('Another Place')
            # display picture
...
##  ----------------------MAIN PAGE ----------------------------------
st.title('ĐI CHƠI ĐÂU ??')
# phan side bar
st.sidebar.header('Welcome to Di choi dau team web')
...
if endpoint == 'Film':
    ...
    with st.container():
        ...
        reday = st.date_input("After which day that you are able to watch movie?", datetime.date(2022, 11, 8))
        max = 15
        count = -1
...
if endpoint == 'Event':
    ...
    with st.container():
        ...
        st.write("##")
        day_picked = st.date_input("Which day that you want to join an event?",  datetime.date(2022, 11, 8))
        event_picked = {
            "Name": [],
            "Location":[],
            "Y/M/D": [],
...
with leftcol:
    st.header("Some suggestion to help our web better")
    st.write("##")
